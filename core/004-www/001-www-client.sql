...
create or replace function www_client.http_post(url text, my_args json)
returns text
as $$
import urllib
import urllib2
...
json_args = json.loads(my_args)
# plpy.log(json_args)
#url = url
#
r = requests.post(url) #, data=_args)
...
begin
    ...
    perform www_client.rows_insert(remote_http_id, bp) from _bundlepacker_tmp bp;
